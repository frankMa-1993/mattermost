...
- [e2e-tests/cypress/.ci/dashboard.generate_test_cycle.sh](file://e2e-tests/cypress/.ci/dashboard.generate_test_cycle.sh)
- [e2e-tests/cypress/.ci/dashboard.start.sh](file://e2e-tests/cypress/.ci/dashboard.start.sh)
- [e2e-tests/cypress/.ci/dashboard.stop.sh](file://e2e......)
</cite>
...
- 国际化
  - 基于 react-intl，通过格式化工具有效提取与校验翻译键值，支持多语言运行时切换。
- 样式系统
...
### 国际化（i18n）
- 多语言支持
  - 基于 react-intl，运行时根据用户语言切换显示文案。
- 文本提取与校验
  - 使用 @formatjs/cli 提取翻译键值，配合 ESLint 插件进行静态校验，避免遗漏与拼写错误。
- 本地化流程
  - 开发阶段定期执行提取脚本，生成/更新翻译文件，CI 中进行校验以保证质量。

```mermaid
flowchart TD
Start(["开始"]) --> Extract["提取翻译键值"]
Extract --> Lint["ESLint 格式校验"]
Lint --> Merge["合并翻译文件"]
Merge --> Validate["运行时校验缺失键"]
Validate --> End(["结束"])
```

**图示来源**
- [webapp/package.json:18-23](file://webapp/package.json#L18-L23)
...
## 结论
本指南从架构、类型、状态、样式、国际化、构建与测试七个维度总结了 Mattermost 前端工程化实践。建议在实际开发中坚持模块化与类型安全，善用 Redux 与 styled-components，配合完善的测试与构建流程，持续提升可维护性与用户体验。
...
- 关键脚本与配置
  ...
  - 国际化：npm run i18n-extract、npm run i18n-extract:check
- E2E 工具链
