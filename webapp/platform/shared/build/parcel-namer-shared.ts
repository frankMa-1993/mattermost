...
export default new Namer({
    async name(opts): Promise<FilePath | null | undefined> {
        ...
        // Use path.relative (not path.posix.relative) so Windows absolute paths resolve correctly.
        const relativeDir = path.relative(path.join(process.cwd(), 'src'), path.dirname(mainEntry.filePath)).replace(/\\/g, '/');
